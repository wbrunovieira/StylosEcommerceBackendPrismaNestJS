...
export class PrismaSizeRepository implements SizeRepository {
  ...
  async findById(id: string): Promise<Size | null> {
    ...
    return record ? Size.create({ name: record.name }, new UniqueEntityID(record.id)) : null;
  }
  ...
  console.log('Pagination params:', { skip, take });
    ...
    return records.map(record => Size.create({ name: record.name }, new UniqueEntityID(record.id)));
  }
}
