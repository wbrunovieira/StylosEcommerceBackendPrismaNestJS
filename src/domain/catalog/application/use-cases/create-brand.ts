  ...
  async execute({
    ...
    try {
      ...
      if (!trimmedName || trimmedName.length === 0) {
        return left(new ResourceNotFoundError("Brand name is required"));
      }
      ...
      const existingBrand = await this.brandRepository.findByName(trimmedName);
      if (existingBrand.isRight()) {
        return left(
          new ResourceNotFoundError("Brand with this name already exists")
        );
      }
