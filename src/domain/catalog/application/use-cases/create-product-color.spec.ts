    ...
    mockProductColorRepository.create = vi.fn(
      async (productId: string, colorId: string) => {
        ...
        if (productOrError.isLeft()) {
          return Promise.resolve(left(new ResourceNotFoundError("Product not found")));
        }
    
        const colorOrError = await mockColorRepository.findById(colorId);
        if (colorOrError.isLeft()) {
          return Promise.resolve(left(new ResourceNotFoundError("Color not found")));
        }
  ...
  it("should create a ProductColor with valid productId and colorId", async () => {
    ...
    if (result.isRight()) {
      ...
      if (createdProductColor) {
        ...
        expect(createdProductColor.colorId.toString()).toBe(colorId.toString());
      }
    } else {
      fail("Expected a Right with the created product color but got Left");
    }
  });
  ...
  it("should not allow duplicate colors for the same product", async () => {
   ...
   if (result.isLeft()) {
      ...
      if (error !== null) {
        ...
        expect(error.message).toBe(`Duplicate color: ${colorId.toString()}`);
      } else {
