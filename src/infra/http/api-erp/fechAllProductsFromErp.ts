...
const createProductSchema = z.object({
  ...
  height: z
    .number()
    .nonnegative('Height must not be negative'),
  width: z
    .number()
    .nonnegative('Width must not be negative'),
  length: z
    .number()
    .nonnegative('Length must not be negative'),
  weight: z
    .number()
...
export class SyncProductsUseCase {
  ...
  async productExists(integrationKey: string, token: string): Promise<boolean> {
    try {
      ...
      return products.some((product: any) => product.integrationKey === integrationKey);
    } catch (error) {
