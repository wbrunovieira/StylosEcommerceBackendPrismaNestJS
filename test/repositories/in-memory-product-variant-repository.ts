  ...
  implements IProductVariantRepository
{
  ...
  async findById(id: string): Promise<Either<ResourceNotFoundError, ProductVariant>> {
    const variant = this.items.find(item => item.id.toString() === id);
    if (!variant) {
      return left(new ResourceNotFoundError(`Product variant not found for ID: ${id}`));
    }
    return right(variant);
  }
  
  async update(variant: ProductVariant): Promise<Either<ResourceNotFoundError, void>> {
    const index = this.items.findIndex(item => item.id.toString() === variant.id.toString());
    if (index === -1) {
      return left(new ResourceNotFoundError(`Product variant not found for ID: ${variant.id}`));
    }
  ...
  async findByProductId(productId: string): Promise<Either<ResourceNotFoundError, ProductVariant[]>> {
    const variants = this.items.filter((item) => item.productId.toString() === productId);
    if (variants.length === 0) {
      return left(new ResourceNotFoundError(`No product variants found for product ID: ${productId}`));
    }
